...
### Currency Awareness Stage 0 assumption map

Current baseline:
- PR #123 で `docs/CURRENCY_AWARENESS_CAMPAIGN_MAP.md` を main に受理済み
- broad FX implementation へ直行せず、Stage 0 current-assumption map から始める
- current pressure point is concrete: daily use may include Israel travel, while the repository still contains JPY / single-amount assumptions

Current authorized finite slice:
- current JPY / single-amount assumptions を repo-wide に docs-only で観察する
- findings を少なくとも source meaning / parsing / arithmetic / aggregation / display / validation / docs-only に分類する
- Stage 1 amount / currency semantics decision に必要な最小 evidence surface を作る
