...
- [x] **Phase 3a: 細い journal add 経路の確立** — `tools/edit-bqn journal add` を本番 `tools/edit` とは別に通す
  - [x] production `tools/edit` は Go fallback のまま維持する
  - [x] Bash dispatcher で Go互換 `journal add` flags を正規化する
  - [x] BQN出力 protocol を2行形式（`OK\tAPPEND\tjournal.tsv` + TSV payload）に固定する
  - [x] BQN validation error は non-zero exit + `ERROR\t<message>` に統一する
  - [x] `journal.tsv` への append を `tools/lib/safe-write.sh` 経由（backup / atomic rename / stale detection）で通す
  - [x] まず resulting TSV bytes の parity を見る（`checks/check-edit-bqn-journal-add.sh`）
- [ ] **Phase 3b: Go/BQN ギャップ解消の設計と実装** — `docs/archive/active-plans/GO_BQN_GAP_ALIGNMENT_PLAN.md`
  - [ ] BQN側: `meta_schema.tsv` の動的パース（予定限定メタデータの動的除外）
  - [ ] Bash側: SHA256スナップショットによる競合（Stale）検知
    - [x] `tools/edit-bqn journal add` narrow path では pre-validation snapshot + write直前再検査を実装済み
  - [ ] Bash側: 置換時の `oldLine` 一致アサーション
...
- `tools/edit-bqn` は2つ目のコマンドを足すまでは narrow proof path として扱い、安易に巨大 dispatcher 化しない。`budget add` 等へ広げる前に、protocol parsing / BQN command invocation / safe append 呼び出しの共通境界を抽出するか判断する。
