...
- [x] Completed exhaustive tracked final-cutover inventory; it measures 65 modules, zero live editor imports, 74 named tests, 33 named checks, 34 old fixture families, eight route/consumer owners, and 35 exactly classified residual runtime references.
...
- [x] Extracted all editor runtime from `src_next` without forwarding wrappers; pure historical Journal/travel semantics live under `src/editor`, shared temporal status under `src/accounting`, and post-write validation is narrow.
...
- [x] Rehearsed deletion/replacement of 401 tracked paths: zero old BQN imports/named path families survive, all 35 runtime and 58 current documentation references have exact actions, and required destination evidence survives.
- [ ] Prepare final route/UI/cache replacement diff without switching daily production.
