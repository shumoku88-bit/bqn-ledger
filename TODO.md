...
- Generic projection ownership inventory A0 is complete as a docs-only audit; record: `docs/archive/audits/GENERIC_PROJECTION_OWNERSHIP_INVENTORY-2026-07-24.md`.
- Detailed completion history through the Journal/MCP checkpoint is in `docs/archive/TODO_HISTORY-2026-07-24.md`.
...
The latest AI working feedback remains evidence only. The generic projection and valuation intake and the completed A0 ownership inventory are design evidence only. Neither source authorizes runtime work without a separately selected finite slice in this file.
...
Status: A0 ownership inventory completed; no next finite slice selected.
...
1. **A1 exact sparse grouping characterization** — test one I/O-free exact key-grouping primitive with public synthetic facts, duplicate-coordinate accumulation, empty-input behavior, and conservation evidence; do not integrate it into production.
2. **A3 Materialize result-contract classification** — docs-only classification of every current `cube.Materialize` output as generic, Canonical, validation, evidence, presentation, or compatibility-only.
3. **B0 commodity and valuation ownership inventory** — document current owners of currency code, source quantity, precision, arithmetic authorization, formatting, mixed-currency rejection, and reporting selection.

Selection rules:

- select at most one candidate first;
- do not refactor `cube.bqn` during docs-only work;
- A1 must remain test-only and must not replace `cube.Materialize`, TBDS, or report routes;
- do not add a universal Cube, projection DSL, Currency axis, FX, valuation, mixed-currency aggregation, source-schema migration, or report output;
- keep the Canonical Daily Cube contract `Day × Account × Layer` unchanged;
- require a separate decision before any production composition or module extraction.
