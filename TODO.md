...
- The multi-currency Journal container contract and test-only admission proof are complete: one Journal may contain multiple supported currencies while every ordinary transaction remains single-domain, transaction domain and scale remain explicit, provenance returns to original source lines, and unlike currencies are never aggregated.
...
The merged test-only proof admits separate balanced JPY and ILS ordinary transactions in one public-synthetic raw Journal, proves the same container boundary with USD, preserves transaction-local domain and calculation scale, remaps successful posting provenance to original Journal lines, rejects mixed-domain ordinary transactions, undeclared transaction domains, and unsupported declarations, and leaves production Stage 1 unchanged. It does not authorize production parser, writer, Stage 2A, context, Cube, TBDS, reports, metadata, source data, accounts, exchange semantics, FX, valuation, or travel changes.
...
Status: contract decision and test-only container proof complete / all production continuation slices independently unselected.
...
1. **Production complete-source admission implementation** — return transaction-domain and calculation-scale evidence without silently widening `historical_external_plan` or manufacturing one Journal-wide domain.
...
- treat the merged single-currency and multi-currency test-only proofs as evidence, not production authorization;
...
Status: non-JPY characterization, single-currency proof, multi-currency Journal container contract, and test-only container proof complete; travel continuation slices remain independently unselected.
...
Status: single-currency characterization and proof, multi-currency Journal container contract, and test-only container proof complete; residual runtime candidates remain unselected.
...
- One test-only Journal can contain separate JPY and ILS ordinary transactions, but current production parsing, writing, Stage 2A, and context do not yet admit them.
