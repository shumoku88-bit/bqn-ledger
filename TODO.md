...
Context:
- 2026-07-08 に `tools/add-ui.sh plan-finish` を既存の `tools/plan-finish-replenish-ui.sh` へ繋ぎ直す tactical fix を実施
- 実績化 + 次回予定補充は使える状態になったが、date / amount / meta 継承 / failure recovery の契約は未整理
- 多通貨関連 PR/workstream を優先してよい。これは次候補として保持する

導線:
- `docs/archive/active-plans/PLAN_COMPLETION_WORKFLOW_DESIGN_INTAKE-2026-07-08.md`

- [ ] docs-only で `Plan Completion Workflow Contract` を設計する
- [ ] `tools/edit plan finish` / `tools/plan-finish-replenish-ui.sh` / `tools/add-ui.sh` / `tools/bl` の責務境界を固定する
- [ ] 実績日・予定日・実績金額差分・次回予定日ルール・meta 継承ルールを決める
