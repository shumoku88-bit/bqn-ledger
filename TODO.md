...
- Ordinary Actual and plan-completion writers leave redundant `currency: JPY` metadata out of native Journal blocks; posting commodities carry the transaction domain.
- Historical implementation plans and completion records remain available under `docs/archive/` and in Git history.

## Things worth exploring

- test whether `layer: actual` can disappear from native Actual transactions without changing behavior;
...
- preview removal of existing reconstructible `currency: JPY` markers after occurrence evidence;
