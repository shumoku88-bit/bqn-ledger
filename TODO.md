...
- [x] Completed reproducible tracked final-cutover inventory; after strict editor Actual extraction it measures 69 modules, 23 live editor import files, 78 named tests, 33 named checks, 35 fixture families, and seven route/consumer owners.
...
- [ ] Extract the remaining 23 live editor files from 10 `src_next` dependencies without forwarding wrappers; editor Actual now uses explicit config, canonical strict Facts, and durable `plan_id` completion only.
