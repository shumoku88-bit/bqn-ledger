...
### daily-flow 表示アライメントと横長レポート対策

- [ ] **全角・半角混在時のアライメントズレの完全解消**
  - 現在の `StringWidth` 判定による CJK 文字幅制御をさらに調査・精緻化し、特定の記号や表示フォントに依存しない完全なズレ対策を行う。
- [ ] **横長レポートの表示切れ対策の検討と実装**
  - 封筒列が多く横幅が広くなった際に画面端で切れてしまう問題に対し、`less -S` による横スクロール表示対応や、折り返し、あるいは列の動的折りたたみなどの対策を検討・実装する。
