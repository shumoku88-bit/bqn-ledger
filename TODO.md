...
- [ ] **Phase 3a: 細い journal add 経路の確立** — `tools/edit-bqn journal add` を本番 `tools/edit` とは別に通す
  - [ ] production `tools/edit` は Go fallback のまま維持する
  - [ ] Bash dispatcher で Go互換 `journal add` flags を正規化する
  - [ ] BQN出力 protocol を2行形式（`OK\tAPPEND\tjournal.tsv` + TSV payload）に固定する
  - [ ] BQN validation error は non-zero exit + `ERROR\t<message>` に統一する
  - [ ] `journal.tsv` への append を `tools/lib/safe-write.sh` 経由（backup / atomic rename / stale detection）で通す
  - [ ] まず resulting TSV bytes の parity を見る
