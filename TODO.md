...
- [ ] **Phase 3c: BQN editor v1 (残りの機能移植と設計)**
  - [ ] 設計: `plan edit` / `journal reverse` のための edit plan プロトコルの詳細設計
  - [ ] 実装: `journal reverse` コマンドの実装とテスト (`checks/check-edit-bqn-journal-reverse.sh`)
  - [ ] 実装: `plan edit` コマンドの実装とテスト (`checks/check-edit-bqn-plan-edit.sh`)
