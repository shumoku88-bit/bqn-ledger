...
`journal.tsv` and `plan.tsv` intentionally contain `budget:*` rows. They must fail lint/strict validation because manual budget allocation belongs in `budget_alloc.tsv`.
