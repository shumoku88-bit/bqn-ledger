...
`checks/check-ledger-facts-phase1-proof-fixture.sh` fixes observable report behavior and normalized-byte equality with destination Trial Balance and Daily Flow human bodies. It compares destination Planned Payments human/JSON with current output for planned `25`, actual `20`, and durable completed identity; destination compact uses the approved `ledger_planned_payment` key only. Canonical tests prove aligned facts, explicit empty source layers, strict source/policy/period definitions, Account-period state, date/category flow, January category rollup, exact sparse Group/Pivot, contributor provenance, and deterministic destination section goldens. Synthetic focused tests cover two-month grouping, temporal Plan states, empty Plan, observation mismatch, and duplicate/conflicting completion evidence.
