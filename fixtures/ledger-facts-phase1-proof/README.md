...
`checks/check-ledger-facts-phase1-proof-fixture.sh` fixes current observable behavior beside destination proofs. Planned Payments compares planned `25`/actual `20`; Account Balances proves all eight Accounts including zero; Recent Journal proves newest-first split debit lanes; Current-cycle Accounts proves the five signed measures through explicit `2026-01-10`. Destination compact keys are only `ledger_planned_payment`, `ledger_balance`, and tab-delimited `ledger_recent_journal`; cycle Accounts is human-only. Canonical tests cover source-qualified provenance, Account periods, sparse Group/Pivot, and Matrix reports. Synthetic focused tests cover ILS mixed-scale, USD empty Actual, unavailable cycle, empty Recent, observation mismatch, and conflicting completion evidence.
