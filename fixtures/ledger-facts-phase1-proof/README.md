...
`checks/check-ledger-facts-phase1-proof-fixture.sh` fixes current observable behavior beside destination proofs. The retained monetary reports preserve bounded questions and deterministic surfaces. Daily Target proves assets `1000`, gross obligation `200`, proven exclusion `50`, once-only deduction `150`, capacity `850`, ten days, and target `85`. Strict destination Issues proves durable/source-ordered open rows, optional date/amount, and exact explicit-currency evidence in `issues.destination.tsv`, which current production intentionally does not read. Synthetic tests also cover deficit, reservation conflict/overflow, Plan completion, expense credits/refunds, under-backed funding, cycle comparisons, empty Actual/Issues, and unavailable evidence.
