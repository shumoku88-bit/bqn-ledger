...
`checks/check-ledger-facts-phase1-proof-fixture.sh` fixes observable report behavior and normalized-byte equality with destination Trial Balance and Daily Flow human bodies. It also runs the canonical Plan completion Join proof beside current Planned JSON assertions for planned `25`, actual `20`, and durable completed identity. Canonical tests prove aligned facts, explicit empty source layers, strict source/policy/period definitions, Account-period state, date/category flow, January category rollup, exact sparse Group/Pivot, contributor provenance, and deterministic destination section goldens. Synthetic focused tests cover two-month grouping plus duplicate/conflicting completion evidence.
