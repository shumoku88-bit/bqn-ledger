...
`checks/check-ledger-facts-phase1-proof-fixture.sh` fixes current observable behavior beside destination proofs. Planned, Balances, Recent, Current-cycle, and Monthly Accounts retain bounded questions. Cycle Comparison proves explicit equal ten-day windows and per-Account difference without inferred baseline search. Destination compact keys remain only `ledger_planned_payment`, `ledger_balance`, and tab-delimited `ledger_recent_journal`; Account Matrix reports are human-only. Canonical tests cover source-qualified provenance and exact reconciliation. Synthetic tests cover 31/28-day complete cycles, ILS mixed scale, USD empty Actual, unavailable cycle, observation mismatch, and conflicting completion evidence.
