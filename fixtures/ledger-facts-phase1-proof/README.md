...
`checks/check-ledger-facts-phase1-proof-fixture.sh` fixes current observable behavior beside destination proofs. It compares Planned Payments human/JSON for planned `25`, actual `20`, and durable completed identity; destination compact uses `ledger_planned_payment` only. Retained Account Balances proves all eight Accounts (including zero), exact JPY closing, source-qualified contributors, and deterministic human/`ledger_balance`/JSON goldens. Canonical tests also cover Account periods, date/category flow, exact sparse Group/Pivot, and Matrix reports. Synthetic focused tests cover ILS mixed-scale and USD empty balances, two-month grouping, temporal Plan states, observation mismatch, and duplicate/conflicting completion evidence.
