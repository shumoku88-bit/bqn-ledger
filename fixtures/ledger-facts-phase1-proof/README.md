...
`checks/check-ledger-facts-phase1-proof-fixture.sh` fixes current observable behavior beside destination proofs. Planned, Balances, Recent, Account Matrices, Comparison, and Envelope & Backing retain bounded questions and deterministic surfaces. Daily Target proves assets `1000`, gross obligation `200`, proven exclusion `50`, once-only deduction `150`, capacity `850`, ten days, and exact target `85` with human/`ledger_daily_target_*` compact goldens. Synthetic tests also cover deficit/shortfall, overdue/empty obligations, duplicate reservation, normalization overflow, open/completed Plans, expense credits/refunds, under-backed funding, complete cycles, mixed scale, empty Actual, and unavailable evidence.
