...
| `docs/JOURNAL_META.md` | native Journal / plan.tsv で使えるメタデータ一覧 |
...
2. **Bash は UI と選択だけを担当する。**
   Bash は `accounts.tsv` / `meta_schema.tsv` / native Journal の意味を解釈しない。
...
6. **BQN-only report path を維持する。**
   source files (configured native Journal and `<base>/*.tsv`; 公開 repo の `data/` は sandbox、実運用は `LEDGER_DATA_DIR` で外出し)、config TSV (`<base>/config.tsv`, `config/system_defaults.tsv`, `config/default_config.tsv`, `config/meta_schema.tsv`) と `src_next/**/*.bqn` で canonical report / export を生成できる状態を保つ。
   `checks/*` は検証用であり、Go editor / Bash UI / gum / fzf / helper scripts / helper-generated cache と同じく、正本レポート計算の必須依存にしない。

## 作業ルール

- AIは、実データ source（base directory 配下のconfigured native Journal, `plan.tsv`, `budget_alloc.tsv`, `accounts.tsv`。例: `LEDGER_DATA_DIR=moko/data`）を絶対に直接編集・新規作成・削除してはならない。公開 repo の `data/` は匿名 sandbox だが、実データと同じ schema のため無用な変更は避ける。特に実運用native Journalを含むこれら正本データへの書き込み操作は、AIのいかなる手段（sed, echo, overwrite, 追記等）を用いても厳禁とする。AIができるのは、あくまで「人間への編集依頼案（diffの提示）」の作成までである。ただしユーザーから明示的に指示をされた場合は編集できるものとする。
- 変更は小さく、1目的ずつ。
...
- base directory 配下のconfigured native Journal / `plan.tsv` / `budget_alloc.tsv` / `accounts.tsv` が正データであり、それ以外は派生であることを忘れないこと。Actual sourceのTSV fallbackを導入してはならない。公開 repo の `data/` は sandbox、実運用は `LEDGER_DATA_DIR` を優先する。個人的な非公開メモや検証ログは root の `private/` ではなく gitignore 済みの `moko/private/` に寄せる。
