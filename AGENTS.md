...
- 設定駆動化を進める場合は `docs/GENERALIZATION_TODO.md` のPhase順と境界を守ること。Canonical Daily Cubeのshape・Layer契約は利用者設定にせず、異なる座標や意味は同じEvent IRから別projection/viewとして作ること。
...
- TUI/GUI/Web UIなどのアプリ外装を作る場合は、`docs/APPLICATION_FOUNDATION.md` を読み、UI状態を source TSV に混ぜず、計算責務をBQNから奪わないこと。
...
設計 doc: `docs/AI_BUDGET_CALCULATOR_DESIGN.md`
