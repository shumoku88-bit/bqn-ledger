...
| **TUI/GUI/Web UIの外装（凍結中）** | `docs/archive/active-plans/APPLICATION_FOUNDATION.md`（凍結中） |
...
6. TUI/GUI/Web UIなどのアプリ外装・操作導線設計（`docs/archive/active-plans/APPLICATION_FOUNDATION.md`, `docs/archive/active-plans/COMMAND_HUB_DESIGN.md`）（凍結中）
...
アプリ外装は、BQN/Go editor の責務を奪わず、表示・選択・入力補助・ナビゲーションに徹する。
...
- TUI/GUI/Web UIなどのアプリ外装を作る場合（※現在凍結中）は、`docs/archive/active-plans/APPLICATION_FOUNDATION.md` を読み、UI状態を source TSV に混ぜず、計算責務をBQNから奪わないこと。
