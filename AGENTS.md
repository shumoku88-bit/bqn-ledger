...
- base directory 配下の明示選択Actual source / `plan.tsv` / `budget_alloc.tsv` / `accounts.tsv` が正データであり、それ以外は派生であることを忘れないこと。Journal modeで`journal.tsv`へfallbackしてはならない。公開 repo の `data/` は sandbox、実運用は `LEDGER_DATA_DIR` を優先する。個人的な非公開メモや検証ログは root の `private/` ではなく gitignore 済みの `moko/private/` に寄せる。
