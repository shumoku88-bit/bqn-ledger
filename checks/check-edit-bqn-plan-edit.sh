...
run_positive_parity() {
  ...
  if ! cmp -s "$go_base/plan.tsv" "$bqn_base/plan.tsv"; then
    echo "FAIL: tools/edit-bqn plan edit result differs from Go editor: $name" >&2
    diff -u "$go_base/plan.tsv" "$bqn_base/plan.tsv" >&2 || true
    exit 1
  fi

  if ! find "$bqn_base/.backup" -type f -name 'plan.tsv*' | grep -q .; then
    echo "FAIL: tools/edit-bqn plan edit did not create a plan backup: $name" >&2
    exit 1
  fi
}
...
./tools/edit-bqn --base "$dry_base" plan edit \
  --id plan-2026-08-15-test-edit \
  --date 2026-08-20 \
  --dry-run \
  --yes \
  --post-check none >/dev/null
assert_unchanged "$dry_base" "$dry_before" "tools/edit-bqn plan edit dry-run"
assert_no_backup "$dry_base" "tools/edit-bqn plan edit dry-run"

# Positive cases.
run_positive_parity edit-by-index-date \
  plan edit \
  --index 2 \
  --date 2026-08-20 \
  --yes \
...
run_positive_parity edit-by-id-amount \
  plan edit \
  --id plan-2026-08-15-test-edit \
  --amount 1234 \
  --yes \
...
run_positive_parity edit-both-date-amount \
  plan edit \
  --id plan-2026-08-15-test-edit \
  --date 2026-08-25 \
  --amount 5678 \
...
run_expect_fail_closed no-change \
  plan edit \
  --id plan-2026-08-15-test-edit \
  --date 2026-08-15 \
  --amount 1000 \
