...
./tools/report-section-metadata >"$work/metadata.tsv"
cmp "$work/metadata.tsv" "$fixture/report_metadata.destination.tsv"
./tools/report-section-metadata --format json >"$work/metadata.json"
...
(
  cd "$work"
  "$root/tools/report-section-metadata" >from-empty-cwd.tsv
)
cmp "$work/from-empty-cwd.tsv" "$work/metadata.tsv"
if ./tools/report-section-metadata --format yaml >"$work/invalid.out" 2>&1; then
...
echo 'check-report-section-metadata: OK'
