...
./tools/report-destination-cache "$base" "$cache" 101 report_all_human.destination.tsv
...
[[ $text_count -eq 10 ]]
./tools/report-destination "$base" all human report_all_human.destination.tsv >"$work/direct-all"
cmp "$work/direct-all" "$cache/all.txt"
./tools/report-destination "$base" balances human JPY 2026-01-12 actual.journal >"$work/direct-balances"
...
if ./tools/report-destination-cache "$base" "$cache" 102 report_all_human.destination.tsv >"$work/lock.out" 2>"$work/lock.err"; then
...
if ./tools/report-destination-cache "$base" "$cache" 102 bad-order.tsv >"$work/bad.out" 2>"$work/bad.err"; then
...
if ./tools/report-destination-cache "$base" "$cache" invalid report_all_human.destination.tsv >"$work/token.out" 2>"$work/token.err"; then
...
echo 'check-report-destination-cache: OK'
