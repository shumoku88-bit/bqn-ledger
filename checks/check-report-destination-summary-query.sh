...
./tools/query-destination "$fixture" "$manifest" --list >"$work/query-list"
cmp "$work/ledger-lines" "$work/query-list"
awk -F': ' '$1 ~ /^ledger_[a-z0-9_]+$/ && !seen[$1]++ {print $1}' "$work/summary" >"$work/keys"
./tools/query-destination "$fixture" "$manifest" --keys >"$work/query-keys"
...
./tools/query-destination "$fixture" "$manifest" ledger_balance >"$work/balances"
cmp "$work/expected-balances" "$work/balances"
[[ $(./tools/query-destination "$fixture" "$manifest" ledger_daily_target_amount) == 81 ]]
...
for key in ledger_bal ledger_balance_extra src_next_balance; do
  if ./tools/query-destination "$fixture" "$manifest" "$key" >"$work/rejected.out" 2>"$work/rejected.err"; then
    echo "FAIL: rejected query succeeded: $key" >&2; exit 1
  fi
  [[ ! -s $work/rejected.out ]]
done
if ./tools/query-destination "$fixture" "$manifest" --grep balance >"$work/rejected.out" 2>"$work/rejected.err"; then
...
echo 'check-report-destination-summary-query: OK'
