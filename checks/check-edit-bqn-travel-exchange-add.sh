...
malformed="$tmp/malformed"; mkdir "$malformed"; cp "$tmp/base/accounts.journal" "$malformed/accounts.journal"; printf 'bad\trow\n' > "$malformed/travel_exchange_events.tsv"
expect_fail_unchanged malformed "$malformed" "${second[@]}"

# Stale append rejects candidate without backup; concurrent marker remains.
stale="$tmp/stale"; mkdir "$stale"; cp "$tmp/base/accounts.journal" "$stale/accounts.journal"; cp "$target" "$stale/travel_exchange_events.tsv"
...
rollback="$tmp/rollback"; mkdir "$rollback"; cp "$tmp/base/accounts.journal" "$rollback/accounts.journal"; cp "$target" "$rollback/travel_exchange_events.tsv"; before="$(sha "$rollback/travel_exchange_events.tsv")"
set +e
...
race="$tmp/race"; mkdir "$race"; cp "$tmp/base/accounts.journal" "$race/accounts.journal"
...
interrupted="$tmp/interrupted"; mkdir "$interrupted"; cp "$tmp/base/accounts.journal" "$interrupted/accounts.journal"
