...
grep -q '^src_next_envelope_execution_planned_remaining: 0$' "$actual_summary"
grep -q '^src_next_envelope_execution_planned_open_total: 0$' "$actual_summary"
...
grep -q '^  現金裏付け未割当 - 予算台帳未割当: 430$' "$human_out"
