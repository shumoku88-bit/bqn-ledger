...
awk -F'\t' '$1=="profit-and-loss" {exit !($4=="2026-01-01" && $5=="2026-01-14")}' "$work/current.tsv"
awk -F'\t' '$1=="cycle-comparison" {exit !($4=="2026-01-13" && $5=="2026-01-03")}' "$work/current.tsv"
awk -F'\t' '$1=="monthly-accounts" {exit !($4=="2026-01" && $5=="2026-02")}' "$work/current.tsv"
awk -F'\t' '$1=="daily-flow" {exit !($6=="2026-01-13")}' "$work/current.tsv"
...
# The daily Command Hub consumes the current profile without modifying the template.
mkdir "$work/ui"
...
# Explicit historical routes remain fixed and reproducible.
...
# A failed refresh retains and labels the last-known-good generation and exposes its diagnostic.
cache="$work/cache"
...
p = Path(sys.argv[1])
s = p.read_text()
old = "daily-flow\thuman\tJPY\t2026-01-01\t2026-02-01\t2026-01-13\tactual.journal"
new = "daily-flow\thuman\tJPY\t2026-01-01\t2026-02-01\t2026-01-12\tactual.journal"
if s.count(old) != 1:
