...
grep -Fq 'cross-cutting shell / UI / experiments / documentation reachability audit' "$queue" \
  || { echo 'FAIL: TODO does not point at the post-BQN cross-cutting audit' >&2; exit 1; }
...
echo "check-bqn-review-queue: production BQN review closed over current ${count} files; no active BQN cursor"
