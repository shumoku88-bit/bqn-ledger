...
Current Envelope membership and stable allocation coordinates have deliberately different lifetimes. `budget.toml` owns the current active Envelope set. Canonical `household.toml [[budget.envelopes]]` owns the stable allocation Account -> Envelope identity relation used by historical Budget movements. Every current Envelope must have a stable coordinate, but a stable coordinate may remain after that Envelope leaves current policy. When historical Envelope evidence is present, every stable coordinate identity must remain in `envelope-history.identities`. Current Budget writers reject new movements through such retired allocation Accounts; readers do not rewrite or discard the old movements.
