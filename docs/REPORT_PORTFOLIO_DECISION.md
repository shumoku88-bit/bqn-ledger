...
| Daily Flow | How did each active accounting Account move on each observed date? | Date × active Account in admitted order |
...
| `daily-flow` | retained as the explicit Actual date × active accounting Account Matrix |
...
- strict Actual/Plan/Entitlement/Envelope/Household admission;
