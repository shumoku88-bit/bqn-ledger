...
validation and canonical Journal block / retained TSV row rendering live in `src_edit/*.bqn`.
...
| entitlement | `entitlement transfer`, `entitlement origin` (`budget add` UI alias) | `tools/entitlement-write` canonical snapshot/publication authority | `src_edit/entitlement_add_cmd.bqn`, `src_edit/entitlement_origin_cmd.bqn`, candidate and validation owners |
