...
| source I/O | `src_next/context.bqn`; `src/application/source_io.bqn` keep-empty helpers | source I/O preserves fields but does not interpret currency or amount unit | B | Stage 1 pressure only if source contract changes |
