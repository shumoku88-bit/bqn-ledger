...
BQN の collection は、要素の shape が互換な場合に表現が同質化し、同じ shape の文字列や配列が高ランク配列としてまとまることがあります。異なる shape の要素はネストしたまま残ることがあるため、要素境界に意味がある値では暗黙表現に依存しないでください。

```bqn
# ❌ 要素境界が必要な値を裸で並べると、入力 shape により表現が揺らぎやすい
statuses ← ⟨"OK", "WARN"⟩

# ✅ 要素境界を固定したい値は明示的に Enclose し、取り出す側で必要な時だけ Disclose する
statuses ← ⟨<"OK", <"WARN"⟩
firstStatus ← >0⊏statuses
```

### 9. `⎊` catch の左オペランドは `𝕩` で失敗させる

入力に対する失敗を `⎊` で受けたい場合、左オペランドは外側の同名・別名変数ではなく引数 `𝕩` を操作します。外側の変数に依存すると、意図した入力境界の外で評価・失敗して catch されないことがあります。

```bqn
outer ← ⟨⟩

# ❌ 123 ではなく outer を見て失敗し、意図した catch 境界から外れる
({1⊑outer}⎊{0}) 123
