...
A declaration-only Journal is a valid empty Actual source. If the selected currency has no Actual transactions, valid selected plan and budget rows still construct the context; Actual contributes zero rows. If Actual, plan, and budget are all empty for the selected currency, the result is a normal empty context. This rule is identical when the Journal contains only other-currency Actual. Invalid evidence still rejects the complete context without partial rows.

When `<base>/config.tsv` declares one valid `DEFAULT_CURRENCY`, the human direct section, the balances body embedded in the full report, and `balances.txt` in the section cache are the same selected-domain body. `all.txt` is byte-identical to direct full-report stdout. Cache generation also writes `.section-keys` in canonical descriptor order, followed by the UI-only `all` cache key; refresh and preview use this manifest instead of duplicating report section arrays. Shell cache refresh does not regenerate or replace balances. Historical fixtures without `DEFAULT_CURRENCY` retain the legacy non-selected balances body in full/cache so unrelated compatibility fixtures remain runnable; direct selected balances remains fail-closed unless `--currency CODE` is explicit. JSON balances and unrelated sections retain their existing contracts.
...
`outlook.BuildAt ⟨ctx,O⟩` prepares actual balances through `actual_snapshot.BuildFromPrepared ⟨ctx.posting_rows,ctx.cube,ctx.resolved,O⟩`. The snapshot is ledger-cumulative through inclusive O and derives numeric balances from checked ledger-wide Posting IR through a local `[O,O+1)` actual-layer TBDS closing view, not an independent Actual-source amount parser. The former broad `actual_snapshot.BuildAt(ctx,O)` compatibility API has no remaining caller and was removed.
