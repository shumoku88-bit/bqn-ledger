...
- `src/sections/monthly_accounts.bqn` — retained human-only Account × Month presentation.
...
The semantic result remains Month × Account so accounting coordinates and contributors are unchanged. Human output transposes that evidence to Account rows × Month columns, because household Account count is much larger than the usual displayed month range. `Range total` is the per-Account range sum; the final `Month total` row is reconciliation evidence. No compact/JSON surface exists.
...
Production routes this human presentation through the retained `monthly-accounts` key.
