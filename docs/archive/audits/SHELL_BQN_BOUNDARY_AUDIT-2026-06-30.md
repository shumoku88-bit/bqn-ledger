...
| journal reverse | `tools/edit-bqn` | currently `src_edit/editor_cmd.bqn` | parse CLI, capture extended APPEND protocol, show original/reversed summary, safe append, post-check | partially | Later `journal_reverse_cmd.bqn` may replace invocation; APPEND apply helper should remain. Summary fields are command-specific preview extras. |
| plan edit | `tools/edit-bqn` | currently `src_edit/editor_cmd.bqn` | parse CLI, capture REPLACE protocol, preview diff, confirm, safe replace, post-check | partially | Later `plan_edit_cmd.bqn` may replace invocation; REPLACE apply helper should remain. |
