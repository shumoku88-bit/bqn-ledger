...
2. ✅ Add a docs-only target policy sketch for future envelope targets (`target_id`, `label`, `selector_key`, `selector_value`) before creating any TSV. Implemented as `docs/archive/active-plans/ENVELOPE_TARGET_POLICY_SKETCH.md` on 2026-06-29.
3. ✅ Inventory residual prefix fallback before code removal. Recorded in `docs/archive/audits/PREFIX_FALLBACK_DEBT_INVENTORY-2026-06-29.md` on 2026-06-29. First cleanup completed: `src_next/balances.bqn` display grouping fallback removed as a separate small change with tests/checks.
