...
| `JOURNAL_POSTING_IR_ADAPTER_PARITY_STAGE2A_PLAN-2026-07-18.md` | active | Selected bounded test-only success-path adapter parity plan. It covers one public synthetic actual and one plan transaction with two postings each and limited semantic parity; identity/provenance, rejection, native multi-posting, production routing, writer, conversion, and cutover remain unselected. |
...
| `REPORT_PROJECTION_ALIGNMENT_PLAN-2026-07-15.md` | active plan | Selected, gated report numeric-owner alignment sequence. Actual Comparison, Outlook Slices A/B, Daily Trend, and Cycle Summary compatibility preparation are complete; Cycle Summary remaining-plan runtime migration is the next eligible candidate but remains unselected. |
| `../completed-plans/CYCLE_REMAINING_PLAN_NUMERIC_OWNER_COMPATIBILITY_DECISION-2026-07-17.md` | historical | Completed docs-only compatibility decision: exclude completed plans, preserve `O <= D < C.end_exclusive`, use a local Posting IR source-row join, and fail Cycle Summary closed on applicable invalid or rejected plan evidence. Runtime migration remains unselected. |
| `../completed-plans/REPORT_SECTION_DESCRIPTOR_CENTRALIZATION_PLAN-2026-07-16.md` | historical | Completed static descriptor centralization with output equivalence and builder validation hardening verified; completion record: `../completed-plans/REPORT_SECTION_DESCRIPTOR_CENTRALIZATION_COMPLETION-2026-07-16.md`. It authorizes no future report work; use current code map and report docs for runtime routing. |
