...
Output order is `event-id`, `layer`, then the mapped fields in the table order. The post-render parity gate constructs that exact expected sequence for every transaction and compares metadata count, ordered keys, ordered values, and presence versus absence, including canonical `event-id`, `layer=actual`, and every present mapped optional value. It also independently retains the Transaction IR `txn_id` and `plan_id` convenience-field comparisons. Any changed key, changed value, missing item, or unexpected item fails as `metadata_parity_failed` with empty canonical bytes. Absence remains absence, and input order does not affect bytes. Unknown, duplicate, empty, unsafe, and malformed controlled metadata fails closed.
...
The declaration preamble emits `commodity JPY` exactly once and every registry account exactly once in lexical account-name order. After parsing, a declaration parity gate compares commodity count/value and the complete ordered declaration set against the explicit external registry: account count, lexically ordered keys, uniqueness, `role`, `kind`, and the existing account `budget` semantic mapped to `default-envelope`. Commodity mismatch returns `commodity_declaration_parity_failed`; any account declaration mismatch returns `account_declaration_parity_failed`, both with empty canonical bytes. `account_resolver_pass` records the account declaration parity result rather than an unconditional value. Stage 2A continues to resolve against the explicit unchanged account registry; there is no automatic account creation or fallback.
...
Failure evidence covers malformed/fewer-field TSV, invalid date and amount, unknown account, duplicate source identity, all description boundaries, unknown/duplicate/empty/malformed metadata, exact metadata value/key/unexpected-item mismatches, declaration role/kind/default-envelope/order mismatches, duplicate commodity declaration, suffix identity overlap, invalid source-file path tokens, ILS, existing output, and concurrent publication. Every semantic failure returns empty canonical bytes, and every command failure publishes no converter bytes.
...
Reconstruction is a separate operation. It reconverts the supplied immutable snapshot, requires the supplied verified prefix to match those canonical bytes exactly, copies suffix bytes without rendering, proves exact-once suffix occurrence, parses and validates the complete candidate through Stage 2A and Stage 2B, and publishes only a new target. Transaction non-overlap is identity-based in this finite public reconstruction: equality of durable `source_event_id` defines overlap across prefix/suffix and within the suffix. Equal date, description, and postings with distinct durable IDs are distinct events; no date/description/posting, amount/account, or description-normalization heuristic is used. Duplicate durable identity returns `suffix_transaction_duplicate`. Public evidence covers a synthetic native three-posting suffix, a prefix identity repeated in the suffix, a duplicate identity within the suffix, and a distinct suffix identity that passes, while proving byte-for-byte suffix preservation. Failure leaves no reconstructed output.

`sourceFileIdentity` is a delimiter-safe token rather than a path. In addition to `/` and `:`, backslash is rejected as `source_file_identity_invalid`; focused tests cover a plain `journal.tsv` token, a slash path, an invented drive-style path, and an invented network-style path.
