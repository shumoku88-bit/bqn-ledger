...
Change historical `src/core/cycle.bqn`'s signature to explicitly accept `as_of_dn` (8-digit integer date):
...
2. Update BQN `ResolveFrom` signature in historical `src/core/cycle.bqn` to accept `as_of` and implement dynamic anchor/calendar month resolving.
