...
1. **Prefix fallback removal decision**:
   Condition: All account references in live ledger TSVs must explicitly declare `role=`. Use counts are detected in `src_next_household_metadata_prefix_fallback_total_count`. Removal parameters are defined in [ACCOUNT_ROLE_CONTRACT.md](ACCOUNT_ROLE_CONTRACT.md).
   Current note: `src_next/projection.bqn` and some report paths already prefer explicit roles, but legacy prefix fallback is still present for compatibility. Keep this item open until the removal decision is made.
...
- [docs/archive/completed-plans/GENERALIZATION_COMPLETED_PHASES.md](GENERALIZATION_COMPLETED_PHASES.md)
