...
- `<base>/<ACTUAL_JOURNAL_FILE>` — `ACTUAL_SOURCE=journal` のActual layer正本。native multi-postingを保持する。
- `<base>/journal.tsv` — `ACTUAL_SOURCE=tsv` compatibility modeだけのActual source。Journal modeでは読まない。
