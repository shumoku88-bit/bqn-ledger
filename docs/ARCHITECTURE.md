...
P4 `cycle_account_period.bqn`はalready-resolved cycleとexplicit observationからinclusive observationのend-exclusive ordinalを確定し、既存`account_period`をcomposeする。raw Posting indicesはこの境界でsource-qualified referenceへ変換される。`sections/cycle_accounts.bqn`はopening/debit/signed credit/movement/closingの五列Matrixをhuman-onlyで所有する。Trial BalanceとDaily Flowは有用なcapability/proofとして残るが、旧routeを自動的に維持する根拠にはならない。production routingと旧surface削除はretained report proofsとsupported-source readiness後のatomic cutoverまで行わない。
