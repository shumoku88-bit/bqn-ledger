...
新facts ownerはsource path、I/O、clock、旧context、Cube/TBDS、report fieldを受け取らない。Phase 1Bの最初のsliceで、complete single-currency admissionのnormalized transaction構造検査は`src/ledger/journal_transaction_structure.bqn`へ移り、`historical_external_plan` parser profileへの依存を除去した。domain partition、exact decimal/account currency proof、complete admission本体はまだ現行ownerにある。test harnessだけがそのcomplete resultを新factsへ外から供給する。schemaとinvariantは`docs/LEDGER_FACT_SCHEMA.md`を正とする。
...
default currency付きfull/cache reportは現在、通常section用の`context.BuildContext`に加えてbalances用selected-domain contextも構築する移行状態にある。両routeのadmission、currency proof、period selectionの契約は同一ではないため、checked rowsや巨大contextへ先に統合しない。selected adapterはcomplete admission済みtransactionsを`cycle.ReadCycleFromAdmittedTransactions`と内部`BuildFromPreparedCore`へ渡し、cycle evidenceと後続compositionの再admissionを除去した。public `BuildFromPrepared`はpolicy-first admissionとfocused first-failure契約を維持する。compatibility `BuildContext`は`actual_source.LoadCycleEvidence`のcomplete-or-historical-fallback evidenceを一度だけ取得し、default/explicit period解決へ再利用する。context-based report consumerは`actual_source.DatesFromContext`を通じて同contextの`actual_transactions`からdate evidenceを得て、focused mock/compatibility contextにそのfieldがない場合だけbase loadへfallbackする。plan completion consumerもcompatibility contextが保持する`historical_external_plan` transaction shapeからdelta-based amount、plan identity、cycle-local IDsを再構成し、Journalを再parseしない。このcompletion seamはselected-domain complete transaction shapeには適用しない。consumer固有のno-dataやobservation policyは各moduleに残す。ただしDaily Flow/Trendのday-count windowとPlanned Payments/Cycle Summaryのexplicit cycle windowは、それぞれconsumer間のexact parityをfixtureで固定した上で`actual_observation.bqn`へ移している。invalid-date、source-order、explicit absence、open-ended frontier policyは共有しない。cycleはlatest-Actual/no-Actual observation、income account、plan evidence、period constructionを引き続き所有する。
