...
Shell は UI・選択・wrapper・safe-write orchestration だけを担当する。`accounts.tsv` / `journal.tsv` / `plan.tsv` / `budget_alloc.tsv` の会計意味や生活ルールは shell に持たせず、候補や検査結果は BQN export / BQN editor protocol / config 由来のものを使う。
