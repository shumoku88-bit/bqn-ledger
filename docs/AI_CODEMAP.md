...
One bounded semantic result per retained question: `envelope_backing`, `account_balances`, `recent_journal`, `planned_payments`, `cycle_accounts`, `cycle_comparison`, `monthly_accounts`, `daily_target`, and `issues`. Renderers consume only their section result.
