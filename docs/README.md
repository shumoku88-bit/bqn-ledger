...
- Journal source migration: resolved envelope assignment persistence は [JOURNAL_RESOLVED_ENVELOPE_ASSIGNMENT_PERSISTENCE_PLAN-2026-07-22.md](archive/completed-plans/JOURNAL_RESOLVED_ENVELOPE_ASSIGNMENT_PERSISTENCE_PLAN-2026-07-22.md) でtest-only完了。account metadataを新規入力時のdefaultとして使い、受理済みの封筒帰属をactual eventへlinkedしたbalanced budget-layer companion eventとして永続化し、後のmetadata変更が過去を再解釈しないことをpublic synthetic evidenceで確認した。次のfinite Journal sliceは未選定（production Journal routing / writer / UI / envelope/report runtime migration / conversion / shadow read / cutover は未選定）
...
| Journal source migration | [Resolved envelope assignment persistence completed record](archive/completed-plans/JOURNAL_RESOLVED_ENVELOPE_ASSIGNMENT_PERSISTENCE_PLAN-2026-07-22.md), [Split-purchase report boundary completed record](archive/completed-plans/JOURNAL_SPLIT_PURCHASE_REPORT_INFORMATION_BOUNDARY_PLAN-2026-07-22.md), [../TODO.md](../TODO.md) | Resolved envelope persistence is test-only completed. Mutable account defaults may propose future assignments, while the persisted companion event owns historical envelope arithmetic. No next Journal slice is selected; production routing, writer/editor, envelope/report runtime migration, conversion, shadow read, and cutover remain unselected. |
