...
- Journal source migration: Stage 2Bまでtest-onlyで完了。次の有限スライスは [JOURNAL_POSTING_IR_COMPARABLE_REJECTION_PARITY_STAGE2C_PLAN.md](JOURNAL_POSTING_IR_COMPARABLE_REJECTION_PARITY_STAGE2C_PLAN.md) の3ケース限定test-only structural rejection parityとして選定済み・未実装。Stage 2B記録は [JOURNAL_POSTING_IR_IDENTITY_PROVENANCE_PARITY_STAGE2B_PLAN-2026-07-19.md](archive/completed-plans/JOURNAL_POSTING_IR_IDENTITY_PROVENANCE_PARITY_STAGE2B_PLAN-2026-07-19.md)。broader red path、native multi-posting、production routing、writer、cutoverは未選定
...
| Journal source migration | [Stage 2C current contract](JOURNAL_POSTING_IR_COMPARABLE_REJECTION_PARITY_STAGE2C_PLAN.md), [../TODO.md](../TODO.md) | Stage 2A/2Bは完了。Stage 2Cは3つのcomparable rejection caseだけをtest-onlyで選定済み・未実装。broader red path、production、native multi-posting、後続stageは自動選定しない。 |
