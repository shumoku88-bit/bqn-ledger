...
- Journal-only Actual source: current contractは [AI_CODEMAP.md](AI_CODEMAP.md), [ARCHITECTURE.md](ARCHITECTURE.md), [DATA_DIR_SETUP.md](DATA_DIR_SETUP.md), [PRODUCTION_EDITOR_DIRECTION.md](PRODUCTION_EDITOR_DIRECTION.md) を読む。Actualは`ACTUAL_JOURNAL_FILE`で指定するnative Journalだけで、TSV route・fallback・dual writeはない。converter、leading-space、profile prerequisitesは [archive/completed-plans/](archive/completed-plans/) の完了記録であり、current runtime指示ではない。private legacy archiveは別承認事項。
