...
- Journal source migration: selected finite plan は [JOURNAL_NATIVE_MULTI_POSTING_APPEND_EDITOR_PLAN.md](JOURNAL_NATIVE_MULTI_POSTING_APPEND_EDITOR_PLAN.md)。owner は native multi-posting Journal entry を次のgoalとして選定した。将来の `journal-block add --journal-file FILE` は独立したexplicit-path writerで、既存 `tools/edit journal add` はTSV-onlyのまま。production source switch / report routing / synchronization / conversion / cutover は未選定
...
| Journal source migration | [JOURNAL_NATIVE_MULTI_POSTING_APPEND_EDITOR_PLAN.md](JOURNAL_NATIVE_MULTI_POSTING_APPEND_EDITOR_PLAN.md), [../TODO.md](../TODO.md) | Status: selected finite production-adjacent explicit-path writer plan. `journal-block add` is separate and explicit-path; existing `tools/edit journal add` remains TSV-only. No production source switch, report routing, synchronization, conversion, or cutover is selected. Exit returns to no selected Journal slice; no later slice is automatic. |
