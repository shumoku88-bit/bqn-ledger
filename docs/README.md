...
- Journal source migration: Stage 2A/2B/2Cとnative three-posting semantic-coordinate parityはtest-onlyで完了。current entryは [JOURNAL_READ_PATH_TRIAL_BALANCE_REHEARSAL_PLAN.md](JOURNAL_READ_PATH_TRIAL_BALANCE_REHEARSAL_PLAN.md) で、既存public Journal fixtureを `Parse -> Stage 2A -> BuildPeriodView -> trial_balance.Build` へ通すtest-only rehearsalがselected / not implemented。production routing、writer、shadow read、conversion、cutoverは未選定
...
| Journal source migration | [Current read-path Trial Balance rehearsal contract](JOURNAL_READ_PATH_TRIAL_BALANCE_REHEARSAL_PLAN.md), [Three-posting semantic-coordinate parity completed record](archive/completed-plans/JOURNAL_NATIVE_THREE_POSTING_SEMANTIC_COORDINATE_PARITY_PLAN-2026-07-19.md), [../TODO.md](../TODO.md) | Stage 2A/2B/2Cとthree-posting parityはtest-onlyで完了。public Journal fixtureを既存period-view / Trial Balanceへ通すrehearsalがselected / not implemented。production route、writer、shadow read、conversion、cutover、後続stageは未選定。 |
