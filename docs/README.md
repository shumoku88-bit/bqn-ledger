...
- Journal source migration: Stage 2A/2B/2C、three-posting parity、rehearsal、resolved-account registry mismatch rejection、split-purchase transaction characterization、および split-purchase report information boundary slice は test-only で完了。完了記録は [JOURNAL_SPLIT_PURCHASE_REPORT_INFORMATION_BOUNDARY_PLAN-2026-07-22.md](archive/completed-plans/JOURNAL_SPLIT_PURCHASE_REPORT_INFORMATION_BOUNDARY_PLAN-2026-07-22.md) を参照。既存split-purchase fixtureを Trial Balance / Balances へ接続し、7 accountの正確な集約と、transaction identity・description・posting orderがsource-side evidenceとして別に残る境界をcharacterizeした（production Journal routing / writer / UI / report redesign / conversion / shadow read / cutover / tax separation は未選定）
...
| Journal source migration | [Split-purchase report boundary completed record](archive/completed-plans/JOURNAL_SPLIT_PURCHASE_REPORT_INFORMATION_BOUNDARY_PLAN-2026-07-22.md), [Split purchase completed record](archive/completed-plans/JOURNAL_SPLIT_PURCHASE_TRANSACTION_CHARACTERIZATION_PLAN-2026-07-22.md), [Read-only carrier completed record](archive/completed-plans/archive/completed-plans/JOURNAL_READ_ONLY_SOURCE_CARRIER_REHEARSAL_PLAN-2026-07-21.md), [../TODO.md](../TODO.md) | split-purchase report aggregation and source-information boundary slice is test-only completed. It preserves exact account-level report values while asserting transaction identity, descriptions, grouping, and posting order only against source-side evidence. Production routing, writer, UI, report redesign, conversion, shadow read, cutover, and tax separation remain unselected. |
