...
- Journal source migration: Stage 2A/2B/2C、three-posting parity、rehearsal、resolved-account registry mismatch rejection、および split-purchase transaction characterization は test-only で完了。次の有限スライスとして [JOURNAL_SPLIT_PURCHASE_REPORT_INFORMATION_BOUNDARY_PLAN.md](JOURNAL_SPLIT_PURCHASE_REPORT_INFORMATION_BOUNDARY_PLAN.md) が選定済み。既存split-purchase fixtureを Trial Balance / Balances へ接続し、7 accountの正確な集約と、transaction identity・description・posting orderがsource-side evidenceとして別に残る境界をcharacterizeする（production Journal routing / writer / UI / report redesign / conversion / shadow read / cutover / tax separation は未選定）
...
| Journal source migration | [Selected split-purchase report boundary plan](JOURNAL_SPLIT_PURCHASE_REPORT_INFORMATION_BOUNDARY_PLAN.md), [Split purchase completed record](archive/completed-plans/JOURNAL_SPLIT_PURCHASE_TRANSACTION_CHARACTERIZATION_PLAN-2026-07-22.md), [Read-only carrier completed record](archive/completed-plans/JOURNAL_READ_ONLY_SOURCE_CARRIER_REHEARSAL_PLAN-2026-07-21.md), [../TODO.md](../TODO.md) | split-purchase report aggregation and source-information boundary is the selected finite test-only slice. It must preserve exact account-level report values while asserting transaction identity, descriptions, grouping, and posting order only against source-side evidence. Production routing, writer, UI, report redesign, conversion, shadow read, cutover, and tax separation remain unselected. |
