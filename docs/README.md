...
- Journal source migration: [JOURNAL_LEGACY_METADATA_PROFILE_EXTENSION_PLAN.md](JOURNAL_LEGACY_METADATA_PROFILE_EXTENSION_PLAN.md) が現在選定中の前提スライス。test-only Minimal BQN Journal profile / Transaction IR / Stage 2A / Stage 2Bで、`source_event_id`とbusiness `txn_id`を分離し、legacy `journal.tsv` metadata vocabularyを明示的に保持する。親の [JOURNAL_CANONICAL_TSV_NATIVE_PREFIX_CONVERTER_PLAN.md](JOURNAL_CANONICAL_TSV_NATIVE_PREFIX_CONVERTER_PLAN.md) は選定済みだがこの前提完了までblocked。converter / reconstruction / production source switch / report routing / default writer / cutoverは変更せず、完了後もconverterやcutoverを自動選定しない。native multi-posting explicit-path append editor の完了記録は [JOURNAL_NATIVE_MULTI_POSTING_APPEND_EDITOR_PLAN-2026-07-22.md](archive/completed-plans/JOURNAL_NATIVE_MULTI_POSTING_APPEND_EDITOR_PLAN-2026-07-22.md)
...
| Journal source migration | [../TODO.md](../TODO.md), [JOURNAL_LEGACY_METADATA_PROFILE_EXTENSION_PLAN.md](JOURNAL_LEGACY_METADATA_PROFILE_EXTENSION_PLAN.md), [JOURNAL_CANONICAL_TSV_NATIVE_PREFIX_CONVERTER_PLAN.md](JOURNAL_CANONICAL_TSV_NATIVE_PREFIX_CONVERTER_PLAN.md) | Status: metadata/profile prerequisite selected; canonical TSV-to-native Journal prefix converter remains selected but blocked. The prerequisite is test-only, keeps the 16-field Posting IR unchanged, and does not change production routing, writers, source truth, conversion, reconstruction, or cutover. |
