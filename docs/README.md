...
- Journal source migration: [JOURNAL_CANONICAL_TSV_NATIVE_PREFIX_CONVERTER_PLAN.md](JOURNAL_CANONICAL_TSV_NATIVE_PREFIX_CONVERTER_PLAN.md) が現在選定中の有限スライス。immutable TSV snapshotからcanonical native Journal historical prefixを生成し、検証済みprefixとbyte-preserved suffixを新しいcandidateへ再構成する一方向contractを定義する。production source switch / report routing / default writer / cutover は変更せず、完了後もcutoverを自動選定しない。native multi-posting explicit-path append editor の完了記録は [JOURNAL_NATIVE_MULTI_POSTING_APPEND_EDITOR_PLAN-2026-07-22.md](archive/completed-plans/JOURNAL_NATIVE_MULTI_POSTING_APPEND_EDITOR_PLAN-2026-07-22.md)
...
| Journal source migration | [../TODO.md](../TODO.md), [JOURNAL_CANONICAL_TSV_NATIVE_PREFIX_CONVERTER_PLAN.md](JOURNAL_CANONICAL_TSV_NATIVE_PREFIX_CONVERTER_PLAN.md) | Status: canonical TSV-to-native Journal prefix converter selected. It defines deterministic legacy topology, identity, metadata, validation, and suffix-preserving reconstruction contracts. Production routing and cutover remain blocked and unchanged; no later slice is selected automatically. |
