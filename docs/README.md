...
- Journal source migration: Stage 2A/2B/2C、native three-posting semantic-coordinate parity、Journal read-path trial-balance rehearsalはtest-onlyで完了。最新の記録は [JOURNAL_READ_PATH_TRIAL_BALANCE_REHEARSAL_PLAN-2026-07-21.md](archive/completed-plans/JOURNAL_READ_PATH_TRIAL_BALANCE_REHEARSAL_PLAN-2026-07-21.md) で、既存public Journal fixtureが `Parse -> Stage 2A -> BuildPeriodView -> trial_balance.Build` を通り、3 Posting IR rowsとzero-sum Trial Balanceを保つevidenceを記録する。次のfinite Journal slice、production routing、writer、shadow read、conversion、cutoverは未選定
...
| Journal source migration | [Read-path Trial Balance rehearsal completed record](archive/completed-plans/JOURNAL_READ_PATH_TRIAL_BALANCE_REHEARSAL_PLAN-2026-07-21.md), [Three-posting semantic-coordinate parity completed record](archive/completed-plans/JOURNAL_NATIVE_THREE_POSTING_SEMANTIC_COORDINATE_PARITY_PLAN-2026-07-19.md), [../TODO.md](../TODO.md) | Stage 2A/2B/2C、three-posting parity、public Journal fixtureから既存period-view / Trial Balanceまでのrehearsalはtest-onlyで完了。次のfinite Journal slice、production route、writer、shadow read、conversion、cutover、後続stageは未選定。 |
