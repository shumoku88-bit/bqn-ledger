...
- Journal source migration: Stage 2A/2B/2Cはtest-onlyで完了。current entryは [JOURNAL_NATIVE_THREE_POSTING_SEMANTIC_COORDINATE_PARITY_PLAN.md](JOURNAL_NATIVE_THREE_POSTING_SEMANTIC_COORDINATE_PARITY_PLAN.md) で、native Journal 3 rowsとlegacy 4 rowsのtopology差を保持したsemantic-coordinate / numeric Cube parity sliceがtest-only selected / not implemented。broader red path、production routing、writer、shadow read、conversion、cutoverは未選定
...
| Journal source migration | [Current three-posting semantic-coordinate parity contract](JOURNAL_NATIVE_THREE_POSTING_SEMANTIC_COORDINATE_PARITY_PLAN.md), [Stage 2C completed record](archive/completed-plans/JOURNAL_POSTING_IR_COMPARABLE_REJECTION_PARITY_STAGE2C_PLAN-2026-07-19.md), [../TODO.md](../TODO.md) | Stage 2A/2B/2Cはtest-onlyで完了。Journal 3 rows / legacy 4 rowsを保持するsemantic-coordinate / numeric Cube parity sliceがtest-only selected / not implemented。broader red path、production route、writer、shadow read、conversion、cutover、後続stageは未選定。 |
