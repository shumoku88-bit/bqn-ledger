...
- Journal source migration: 現在の選定済みfinite sliceは [JOURNAL_RESOLVED_ENVELOPE_ASSIGNMENT_PERSISTENCE_PLAN.md](JOURNAL_RESOLVED_ENVELOPE_ASSIGNMENT_PERSISTENCE_PLAN.md)。account metadataを新規入力時のdefaultとして使い、受理済みの封筒帰属をactual eventへlinkedしたbalanced budget-layer companion eventとして永続化し、後のmetadata変更が過去を再解釈しないことをpublic synthetic evidenceでcharacterizeする。直前のsplit-purchase report boundaryは [JOURNAL_SPLIT_PURCHASE_REPORT_INFORMATION_BOUNDARY_PLAN-2026-07-22.md](archive/completed-plans/JOURNAL_SPLIT_PURCHASE_REPORT_INFORMATION_BOUNDARY_PLAN-2026-07-22.md) でtest-only完了（production Journal routing / writer / UI / envelope/report runtime migration / conversion / shadow read / cutover は未選定）
...
| Journal source migration | [Resolved envelope assignment persistence selected plan](JOURNAL_RESOLVED_ENVELOPE_ASSIGNMENT_PERSISTENCE_PLAN.md), [Split-purchase report boundary completed record](archive/completed-plans/JOURNAL_SPLIT_PURCHASE_REPORT_INFORMATION_BOUNDARY_PLAN-2026-07-22.md), [../TODO.md](../TODO.md) | Selected test-only slice persists the accepted envelope effect as a balanced budget-layer companion event linked to the actual event. Mutable account defaults may propose future assignments but must not reinterpret historical envelope consumption. Production routing, writer/editor, envelope/report runtime migration, conversion, shadow read, and cutover remain unselected. |
| 封筒予算 | [JOURNAL_RESOLVED_ENVELOPE_ASSIGNMENT_PERSISTENCE_PLAN.md](JOURNAL_RESOLVED_ENVELOPE_ASSIGNMENT_PERSISTENCE_PLAN.md), [ENVELOPE_ROLE_DESIGN.md](ENVELOPE_ROLE_DESIGN.md), [ENVELOPE_FUNDING_BASE_INVARIANT.md](ENVELOPE_FUNDING_BASE_INVARIANT.md), [ENVELOPE_EXECUTION_AND_PLAN_POLICY.md](ENVELOPE_EXECUTION_AND_PLAN_POLICY.md) | 現在のJournal sliceはresolved assignmentの履歴安定性だけをtest-onlyで選定。runtime運用細部は adjustment / seed / budget_pool policy を参照。 |
