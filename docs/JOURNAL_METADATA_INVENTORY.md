...
Plan Finish publishes an identity-free Actual carrying the unique canonical `plan-id` relation. The completion candidate does not persist the reconstructible `completion-<plan-id>-<actual-date>` transport identity. Historical Budget execution companions may retain an existing durable `actual-event-id`, but Plan completion no longer creates or requires such a companion; current Fulfillment uses the unique PlanId relation.
...
| `plan-id` | Plan editor and plan completion workflow; explicit metadata append | Parser completion links, plan completion validation, `actual_source.CompletionEvidence`, historical companion admission, identity inventory | Sometimes visible inside a completion `event-id`, but not universally and not safely for arbitrary imported data | Keep. It is a real external relationship between a plan row and an Actual transaction. |
...
| `actual-event-id` | Historical/test Budget companion linked to a durable Actual event | Historical parser validation and identity inventory | Not derivable from companion postings because several Actual events could have the same date, accounts, and amount | Retain on historical linked companions; no current Plan completion writer creates one. |
