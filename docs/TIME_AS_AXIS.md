...
現行 `src_next` では、section ごとの観察境界を一律に「すべて `as_of` で切られる」と説明してはいけない。cycle summary / TBDS / actual snapshot / recent rows / planned payments / actual comparison はそれぞれ入力境界と表示目的が異なる。現行挙動を確認する場合は、`src_next/report.bqn`、該当 section module、`docs/REPORT_CONTRACTS.md`、および `checks/check-src-next-*.sh` を正とする。
