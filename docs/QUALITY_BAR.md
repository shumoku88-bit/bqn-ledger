...
- 1目的1差分を基本にする。
- source migration / default switch は明示gateとrollback evidenceなしに急がない。
...
- **エラー出力の隠蔽厳禁**: テストコマンド (`bqn`, `bash checks/...`, `tools/check.sh` など) の出力を `>/dev/null` で捨ててはならない。失敗した時に「なぜ落ちたか」が隠されてしまい、CIデバッグが困難になる罠を防ぐ。
