...
| `daily-flow` | implemented date/category report; final P1 contract retains it despite an earlier provisional retirement note | old and destination flow owners | `src/sections/daily_flow.bqn` | retained current report | yes | yes | yes | 1 | yes | trust current 12-key contract, not provisional nine-key text |
